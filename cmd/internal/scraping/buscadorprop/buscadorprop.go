...
func MakeScraping(url []string) ([]scraping.Department, error) {
	...
	for _, pageUrl := range url {
		...
		task := chromedp.Tasks{
			...
			chromedp.WaitVisible(`.card`),
			chromedp.OuterHTML("#listadoPropiedades", &source, chromedp.NodeVisible, chromedp.ByID),
		}
		...
		page.Find(".card").Each(func(i int, s *goquery.Selection) {
...
func GetAddress(s *goquery.Selection) string {
	return strings.TrimSpace(s.Find(".card__location").Text())
}
...
func GetDetails(s *goquery.Selection) string {
	return strings.TrimSpace(s.Find(".card__description").Text())
}

func GetPrice(s *goquery.Selection) string {
	return strings.TrimSpace(s.Find(".card__price__actual").Text())
}

func GetLinkToDetail(s *goquery.Selection) string {
	link, _ := s.Find(".card__buttons > a:nth-child(2)").Attr("href")
	return strings.TrimSpace(link)
}

func GetImageLink(s *goquery.Selection) string {
	styleAttr, _ := s.Find(".card__image__photo").Attr("style")
	if styleAttr == "" {
		return ""
	}
	urlParsed := strings.Split(styleAttr, `'`)
	return urlParsed[1]
}
